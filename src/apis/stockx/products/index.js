...
export default class Products extends Base {
  async search(query, options = {}) {
    if (!query){
      const err = new Error("No search query specified!");
      err.status = 404;
      err.body = '';
      throw err;
    }
    ...
    const { limit } = options;
    const { currency, headers, proxy, request } = this.context;
    const url = `https://stockx.com/api/browser?&_search=${query}&dataType=${type}&currency=${currency}`;
